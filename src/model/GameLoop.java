...
public class GameLoop extends Thread {
	...
	private void doGameLoop() {
		...
		if(ball.getX() != rightPaddle.getX() || ball.getY() >= rightPaddle.getY() - (rightPaddle.getHeight() / 2) &&
				 (ball.getY() != 20) && (ball.getY() != gameHeight - 20))
		{
			//If touching left paddle test
			if((ball.getX() != leftPaddle.getX() + (ball.getHeight() /2) && ball.getY() != leftPaddle.getY()))
			{
				//If touching right paddle test
				if(ball.getX() != rightPaddle.getX() && ball.getY() >= rightPaddle.getY() - (rightPaddle.getHeight() / 2) && ball.getY() <= rightPaddle.getY() + (rightPaddle.getHeight() / 2))
				{
					//If touching left or right side tests
					if((ball.getX() != gameWidth - 20) && (ball.getX() != 0))
					{
